...
Разметка Qt-окон и страниц хранится в `src/qt/widgets/*.ui` и обрабатывается
`AUTOUIC` при сборке `chat_qt_ui`. Классы виджетов вызывают `setupUi` и связывают
элементы с контроллером; сетевой worker и общий клиентский API не зависят от форм.
