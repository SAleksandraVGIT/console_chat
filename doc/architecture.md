...
- `ChatService` (`include/console_chat/core/chat_service.h`, `src/core/chat_service.cpp`)
  ...
  - может получить `Logger` через `SetLogger`;
  - после успешной отправки сообщения пишет одну строку лога, если логгер подключён.

`ChatService` не хранит глобальную авторизацию процесса; текущий логин передаётся в методы явно.

### Logging (`namespace console_chat::logging`)

- `Logger`
  - открывает файл логов в конструкторе и закрывает его в деструкторе;
  - реализует RAII для файловых потоков;
  - `WriteLine` потокобезопасно добавляет одну строку в файл;
  - `ReadLine` потокобезопасно читает одну строку из файла;
  - скрывает `std::mutex` и детали работы с файлом от остального кода.

Сервер создаёт `Logger` только при наличии параметра `--log-file`.
Без этого параметра логгер не создаётся, а `ChatService` работает без логирования.
