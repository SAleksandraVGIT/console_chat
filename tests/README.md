...
- `ClientConfigTest.*`: дефолтный интервал, загрузка файла, границы значений,
  неверные ключи и дубликаты.
- `LiveView.*` (POSIX): изолированный компонент с подставным источником снимков в
  псевдотерминале; сохранение текста и курсора при обновлении, редактирование UTF-8,
  изменение размера окна и прокрутка, EOF, восстановление `termios` после ошибки,
  отсутствие активности и лишней перерисовки при неизменных данных фонового опроса.
  Эти проверки не запускают приложение или сервер.
- `RequestFlow.PollOnlyAllowsAuthenticatedReadOperations`: права доступа к приватным
  чатам и админским запросам, запрет изменяющих и вложенных команд через `POLL`.
- `ClientServerE2E.PollObservesNewUsersChatsMessagesAndAdminChanges`: новые пользователи,
  приватные чаты, сообщения, GENERAL от ADMIN и удаление аккаунта видны при следующем опросе.
- `ClientServerE2E.PollDoesNotPreventIdleDisconnect`: постоянный опрос не отменяет таймаут.
- `ClientServerE2E.UserActivityExtendsIdleDeadline`: ввод продлевает сессию, затем
  отсутствие активности приводит к отключению даже без нового запроса.
- `ClientServerE2E.PollReportsAdminDisconnect`: клиент получает причину отключения при опросе.
